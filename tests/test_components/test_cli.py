...
def test_config(tmp_path):
    ...
    result = runner.invoke(
        cli.config, ["--update", "--config_folder", tmp_path, "--no-add-projects"]
    )
    assert result.exit_code == 0
    prm = config_tools.load_param(param_folder=tmp_path)
    assert len(prm["project_ids"]) == len(config_tools.DEFAULT_CONFIG["project_ids"])
...
def test_create_yaml(tmp_path):
    ...
    result = runner.invoke(cli.create_yaml, ["-s", path2yaml, "-t", out_yml])
    assert result.exit_code == 0
...
def test_make_full_yaml(tmp_path):
    ...
    result = runner.invoke(
        cli.process_yaml, ["-s", path2yaml, "-t", out_yml, "--overwrite"]
    )
    assert result.exit_code == 0
    # read auto yaml
    with open(out_yml, "r") as reader:
...
def test_upload(tmp_path):
    ...
    result = runner.invoke(
        cli.yaml_to_flexilims, ["-s", out_yml, "--conflicts", "skip"]
    )
    assert result.exit_code == 0
...
def test_flm_issues(tmp_path):
    ...
    runner = CliRunner()
    result = runner.invoke(
        cli.check_flexilims_issues, ["-t", out_csv, "-p", TEST_PROJECT]
    )
    assert result.exit_code == 0
